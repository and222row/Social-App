...
import { addPost, getUserPosts } from "@/lib/Redux/PostsSlice";
      ...
      if (selectedImage) {
        dispatch(addPost({ body: values.postText, image: selectedImage }))
          .then(() => {
            ...
            setImageName("");
            toast.success("Post is added, check your Profile", {
              position: "top-center",
            });
            dispatch(getUserPosts(localStorage.getItem("userId")));
          })
          .catch((error) => {
            console.error("Submission error: ", error);
          });
      } else {
        toast.error("image and content can't be empty", {
          position: "top-center",
        });
      }
    },
        ...
        <Box className={styles.actions}>
          <Box className={styles.uploadContainer}>
            ...
            <Box className={styles.fileNames}>
              {imageName && (
                <Typography className={styles.fileName}>{imageName}</Typography>
              )}
            </Box>
          </Box>
          <Button
